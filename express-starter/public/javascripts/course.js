...
async function loadReviews() {
  ...
  try {
    ...
    if (badge) badge.textContent = allReviews.length || '';
...
function renderReviews() {
  ...
  const filtered = allReviews.filter(r => minRating === 0 || r.overallRating >= minRating);

  if (filtered.length === 0) {
    reviewsList.innerHTML = '<p class="panel-empty">No reviews match this filter.</p>';
    return;
  }

  reviewsList.innerHTML = '<ul>' + filtered.map((r) => `
    <li>
      <p>
